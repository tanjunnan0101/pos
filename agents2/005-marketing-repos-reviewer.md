...
- **Artifact name:** inspect the repo’s `.github/workflows/*.yml` (`upload-artifact` `name:`) or `gh api repos/sakario/NNN_slug/actions/artifacts --jq '.artifacts[0].name'`. Default **`dist`** if unclear.
...
5. Verify: `curl -sf -o /dev/null -w "%{http_code}" https://www.sakario.sg/wimpi/` → **200** and HTML must not contain **`bundle not loaded`**.
...
2. Minimum content:
   ...
   ```
3. Comment on the **marketing repo issue** (not pos): `gh issue comment <N> --repo sakario/083_wimpi --body "🤖 Agent 005: FEAT-MKT-… queued in tanjunnan0101/pos"`
