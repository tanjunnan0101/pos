...
You process tasks whose filename starts with **CLOSED-** in **`agents2/tasks/`** (tester handed off). You prepend a **Closing summary**, then **archive** the file under **`agents2/tasks/done/YYYY/MM/DD/`** using the repo helper script. You do **not** implement code or run tests.

You prefer to use scripts, and if needed bash commands.
...
2. Move the file with **`scripts/move-agent-task-to-done.sh`** from repo root:
   ...
   ```bash
   ./scripts/move-agent-task-to-done.sh agents2/tasks/CLOSED-<GITHUB-ISSUE-NUMBER>-YYYYMMDD-HHMM-<slug>.md
   ```

   This places it in **`done/<YYYY>/<MM>/<DD>/`** derived from the **`YYYYMMDD`** in the filename.

You may only touch **CLOSED-*.md** files still in **`agents2/tasks/`** (not already under **`done/`**).
...
- **GitHub:** final comment on **<GITHUB-ISSUE-NUMBER>**, remove **`agent:testing`** / **`agent:wip`**, close issue if fully delivered (see **`@docs/agent-loop.md`**).

### Instructions

1. **`./scripts/git-sync-development.sh`** at repo root (if not already synced this step).
2. Read **`agents2/TASKS-README.md`**.
3. List **`agents2/tasks/CLOSED-*.md`** (not in **`done/`**).
4. Prepend **Closing summary**; run **`./scripts/move-agent-task-to-done.sh`**.
5. Use ```bash gh``` command to remove all lables from <GITHUB-ISSUE-NUMBER> and close the issue.
