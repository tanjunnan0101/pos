...
## Test report (2026-04-27T14:44Z–2026-04-27T14:47Z UTC — `020-test` tester)

1. **Date/time (UTC) and log window**  
   - **Window:** `2026-04-27T14:44Z`–`2026-04-27T14:47Z`.  
   - **Commands:** `./scripts/git-sync-development.sh`, `git fetch origin`, `git rev-parse`, `git merge-base --is-ancestor`, `gh run list` / `gh run view`, `curl -w '%{http_code}'` from `/Users/raro42/projects/pos2` (not `docker logs` — not required by Testing instructions).

2. **Environment**  
   - **Compose / local app:** N/A.  
   - **Branch:** `development` after `./scripts/git-sync-development.sh` (`Already up to date` with `origin/development`).  
   - **Remotes (after `git fetch origin`):** `origin/master` = **`7a2c2bd59b2cfb6cbc6a55ac407993494b17256f`**, `origin/development` = **`7cb3726a2e15c2dd2e64ce746004145fc63395bd`**.  
   - **Evidence:** GitHub Actions via `gh`; HTTP via `curl`.

3. **What was tested (from Testing instructions)**  
   - (1) `git rev-parse origin/master origin/development` and `merge-base` ancestor check.  
   - (2) **Deploy to amvara9** — latest **`master`** push workflow vs **24773000757** / any newer **success**.  
   - (3) Optional live check after **green** — **N/A** (no green **`master`** deploy).  
   - (4) Manual server deploy — **N/A** (not run).
...
4. **Results**
   ...
   |---|---|---|
   | 1. Git: refs / lineage | **PASS** | SHAs above; `git merge-base --is-ancestor origin/master origin/development` → exit **0**. |
   | 2. GitHub Actions: **green** latest **`master`** **Deploy to amvara9** | **FAIL** | `gh run list --repo satisfecho/pos --workflow "Deploy to amvara9" --branch master --limit 8`: newest **`master`** run **`24773000757`** (**completed**, **failure**, **2026-04-22T10:18:20Z**). `gh run view 24773000757 --json conclusion,headSha,databaseId,updatedAt` → **`failure`**, **`headSha`** = **`7a2c2bd59b2cfb6cbc6a55ac407993494b17256f`** (`databaseId` **24773000757**, **`updatedAt`** **2026-04-22T10:18:30Z**). No newer **`master`** **success** after this run in the listed window (next older successes are **24708658534**, **24708247767**, etc.). |
   | 3. Optional after green | **N/A** | Blocked until (2) passes. |
   | 4. Manual fallback | **N/A** | Not run. |
   | Sanity (HTTP) | **INFO** | `curl` → `https://satisfecho.de/api/health` **200**, `https://satisfecho.de/` **200** — does not prove green CI. |
...
6. **Product owner feedback**  
   **`origin/master`** is still contained in **`origin/development`** history, but GitHub **Deploy to amvara9** for the current **`master`** tip remains run **24773000757** (**failure**); issue **#195** “confirm deployment action succeeded” is **not** satisfied until a **success** conclusion appears (marketing **Actions** secrets / re-run per implementation summary). Live site **200** responses only show the edge is up, not a green pipeline.
...
8. **Relevant log excerpts (last section)**  
   - `gh run view 24773000757 --repo satisfecho/pos --json conclusion,updatedAt,headSha,databaseId` → **`"conclusion":"failure"`**, **`"updatedAt":"2026-04-22T10:18:30Z"`**, **`"headSha":"7a2c2bd59b2cfb6cbc6a55ac407993494b17256f"`**, **`"databaseId":24773000757`**.  
   - **Loop protection (`020-test.md`):** Same workflow run **ID** and **`updatedAt`** as prior reports — verification cannot change outcome without a new **success** run (secrets fixed + re-run or new **`master`** deploy). **Not** additional automated retries in this session beyond this single evidence pass.
