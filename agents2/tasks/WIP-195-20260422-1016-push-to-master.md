...
## Test report (2026-04-27T17:09Z–2026-04-27T17:13Z UTC, 020-tester)

1. **Date/time (UTC) and log window**  
   - **Window:** 2026-04-27T17:09Z–2026-04-27T17:13Z.  
   - **Evidence:** `./scripts/git-sync-development.sh` at session start; `git fetch` / `git rev-parse` / `git merge-base --is-ancestor`; `gh run list` (recent runs); `gh run view 24773000757 --json`; `curl` to prod — host repo `/Users/raro42/projects/pos2`. Not `docker logs` (not required by Testing instructions).

2. **Environment**  
   - **Compose / local stack:** N/A.  
   - **Branch:** `development` (synced; **Already up to date** at sync script run).  
   - **After `git fetch origin`:** `origin/master` = `7a2c2bd59b2cfb6cbc6a55ac407993494b17256f`, `origin/development` = `b0ae6df81169176ef191608a23598134aa7479a9`.

3. **What was tested (from Testing instructions)**  
   - (1) `git rev-parse` for both remotes; **`origin/master`** an ancestor of **`origin/development`**.  
   - (2) **Deploy to amvara9** — latest **`master`** run vs **green** (polled via `gh run list` / `gh run view`).  
...
4. **Results**
   ...
   | Criterion | Result | Evidence line |
   |---|---|---|
   | 1. Git: refs / lineage | **PASS** | `git merge-base --is-ancestor origin/master origin/development` → exit **0**; tips **`7a2c2bd…`** / **`b0ae6df…`**. |
   | 2. GitHub Actions: **green** latest **`master`** **Deploy to amvara9** | **FAIL** | `gh run list --repo satisfecho/pos --limit 15` — newest **`headBranch":"master"`** **Deploy to amvara9** is **`databaseId":24773000757`** (**2026-04-22T10:18:20Z**), **`conclusion":"failure"`**. `gh run view 24773000757 --json conclusion,headSha` → **`failure`**, **`headSha`** matches **`origin/master`**. No newer **`master`** success supersedes this in recent workflow history. |
   | 3. Optional after green | **N/A** | Blocked until (2) passes. |
   | 4. Manual fallback | **N/A** | Not run. |
   | Sanity (HTTP) | **INFO** | `curl` → `https://satisfecho.de/api/health` **200**, `https://satisfecho.de/` **200** — does not prove CI green. |

5. **Overall:** **FAIL** — criterion **2** fails ( **`master`** deploy **24773000757** still **failure**); criterion **1** passes.

6. **Product owner feedback**  
   **`development`** has moved ahead of **`master`** as expected after promotion; **`master`** tip remains **`7a2c2bd…`**. The Actions pipeline for that **`master`** push has still not been superseded by a **green** run — operators must fix marketing artifact secrets / PAT scope and re-run or redeploy before this issue can pass automated verification.
...
8. **Relevant log excerpts (last section)**  
   - `gh run view 24773000757 --repo satisfecho/pos --json conclusion,headSha,updatedAt` → **`"conclusion":"failure"`**, **`headSha":"7a2c2bd59b2cfb6cbc6a55ac407993494b17256f"`**, **`updatedAt":"2026-04-22T10:18:30Z"`**.  
   - **Loop protection:** Same blocker as prior sessions (failed **`master`** deploy **24773000757**); further identical git-only checks do not change outcome until CI succeeds.
