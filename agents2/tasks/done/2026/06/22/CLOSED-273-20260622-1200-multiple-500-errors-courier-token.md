...
## GitHub Issues
- **Issue:** https://github.com/tanjunnan0101/pos/issues/273
- **273**

## Problem / goal

Production is returning **500 Internal Server Error** on repeated `POST /token?scope=courier` requests (reported roughly every 30 minutes). Courier driver login at `/courier/login` cannot authenticate on **sakario.sg**.
...
- Reproduce on production: `POST https://www.sakario.sg/api/token?scope=courier` → expect **500** today; confirm amvara9 `pos-back` log shows `invalid input value for enum userrole: "courier"`.
...
- Coordinate with **#272** deploy task: after fix lands on **`development`**, production promotion + amvara9 deploy should be re-run; do not leave **`master`** / sakario.sg broken.
...
2. **Environment:** Local `docker-compose.yml` + `docker-compose.dev.yml`, `BASE_URL=http://127.0.0.1:4202`, branch `development` (working tree). Production `https://www.sakario.sg`. Latest **Deploy to amvara9** run: https://github.com/tanjunnan0101/pos/actions/runs/27912680055 (**success**, commit `7405465c`, 2026-06-21 — predates fix **20260621120000**).
...
4. **Results:**
   ...
   - **Production courier token:** **FAIL** — `POST https://www.sakario.sg/api/token?scope=courier` → **500**; amvara9 `pos-back`: `invalid input value for enum userrole: "courier"`.
...
2. **Environment:** Local `docker-compose.yml` + `docker-compose.dev.yml`, `BASE_URL=http://127.0.0.1:4202`, branch **`development`** @ **`0f6ba00b`** (fix committed). Production `https://www.sakario.sg`. Latest **Deploy to amvara9** run: https://github.com/tanjunnan0101/pos/actions/runs/27912680055 (**success**, commit **`7405465c`**, 2026-06-21 — predates fix; no newer deploy).
...
2. **Environment:** Local `docker-compose.yml` + `docker-compose.dev.yml`, `BASE_URL=http://127.0.0.1:4202`, branch **`development`** @ **`0f6ba00b`**. Production `https://www.sakario.sg`. Latest **Deploy to amvara9** run: https://github.com/tanjunnan0101/pos/actions/runs/27912680055 (**success**, commit **`7405465c`**, 2026-06-21T17:54:20Z — predates fix **`0f6ba00b`**; no newer deploy).
...
2. **Environment:** Local `docker-compose.yml` + `docker-compose.dev.yml`, `BASE_URL=http://127.0.0.1:4202`, branch **`development`** @ **`274db879`**. Production `https://www.sakario.sg`. Latest **Deploy to amvara9** run: https://github.com/tanjunnan0101/pos/actions/runs/27912680055 (**success**, commit **`7405465c`**, 2026-06-21T17:54:20Z — predates fix **`0f6ba00b`**; no newer deploy). **`master`** tip: **`7405465c`**.
...
2. **Environment:** Local `docker-compose.yml` + `docker-compose.dev.yml`, `BASE_URL=http://127.0.0.1:4202`, branch **`development`** @ **`45cb8617`**. Production `https://www.sakario.sg`. Latest **Deploy to amvara9** run: https://github.com/tanjunnan0101/pos/actions/runs/27912680055 (**success**, commit **`7405465c`**, 2026-06-21T17:54:20Z — predates fix **`0f6ba00b`**; no newer deploy). **`master`** / amvara9 tip: **`7405465c`**. Fix commit **`0f6ba00b`** is on **`development`** but **not** on **`master`**.
