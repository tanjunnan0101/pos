...
- **What was done:** In `sakario/040_gustazo`, deleted `public/gallery/local/local-04.jpeg`, removed the entry from `gallery.data.ts`, and pushed `e178aaa` to `main`; POS **Deploy to amvara9** run 26770417180 synced the new `gustazo-dist` bundle.
...
## GitHub
- **Issue:** https://github.com/sakario/040_gustazo/issues/1
- **Marketing repo:** sakario/040_gustazo
- **MKT-040-1**
- **Live path:** https://www.sakario.sg/gustazo/
...
2. **Environment:** Branch `development` (pos repo, synced). Production `BASE_URL=https://www.sakario.sg/gustazo/`. Marketing repo `sakario/040_gustazo` @ `e178aaa`.
...
4. **Results:**
   - CI green + `gustazo-dist` artifact uploaded — **PASS** — https://github.com/sakario/040_gustazo/actions/runs/26769994461 (success, headSha `e178aaa`, 123 files uploaded).
   - Production updated after deploy sync (no `local-04` in live bundle) — **FAIL** — Live JS is `main-RIJZXBRL.js`; `curl …/main-RIJZXBRL.js | grep local-04` returns 1 match. Last pos **Deploy to amvara9** run https://github.com/tanjunnan0101/pos/actions/runs/26769295375 completed `2026-06-01T16:57:17Z`, **before** gustazo CI at `17:11:05Z`.
...
7. **URLs tested:**
   1. https://www.sakario.sg/gustazo/
   2. https://www.sakario.sg/gustazo/gallery/local/local-04.jpeg (direct probe — 200, orphaned asset still on server)
   3. https://www.sakario.sg/gustazo/main-RIJZXBRL.js (live bundle probe)

8. **Relevant log excerpts:**
   - Gustazo CI: `Artifact name is valid!` / `name: gustazo-dist` / `Uploaded bytes …` (run 26769994461).
   - Production network (Chrome DevTools): `GET https://www.sakario.sg/gustazo/gallery/local/local-04.jpeg [200]` after expanding gallery.
...
4. **Results:**
   - CI green + `gustazo-dist` artifact uploaded — **PASS** — https://github.com/sakario/040_gustazo/actions/runs/26769994461 (success, headSha `e178aaa`, artifact `gustazo-dist` ID 7337998544, 45.8 MB).
   - Production updated after deploy sync (no `local-04` in live bundle) — **PASS** — Deploy run https://github.com/tanjunnan0101/pos/actions/runs/26770417180 completed `2026-06-01T17:19:37Z` (after gustazo CI). Live JS is `main-3AJA4E6Q.js`; `curl …/main-3AJA4E6Q.js | grep local-04` → 0 matches. Direct probe `/gallery/local/local-04.jpeg` → **404**.
...
7. **URLs tested:**
   1. https://www.sakario.sg/gustazo/
   2. https://www.sakario.sg/gustazo/gallery/local/local-04.jpeg (direct probe — 404)
   3. https://www.sakario.sg/gustazo/main-3AJA4E6Q.js (live bundle probe)
