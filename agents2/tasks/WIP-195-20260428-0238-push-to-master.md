...
*(Prior session noted closure to **WIP** but filename had returned to **UNTESTED**; this session completes **TESTING** → **WIP**.)*

---

## Test report

**Date/time (UTC):** 2026-04-28 20:33 UTC  
**Log window:** `./scripts/git-sync-development.sh`; `git fetch origin`; `gh run list` / `gh run view`; `curl` to `https://satisfecho.de/api/health` (same session, ~3 minutes).

**Environment:** Repo root **`/Users/raro42/projects/pos2`** (synced **`development`**); Docker / compose app tests **N/A**. **`BASE_URL` (optional):** `https://satisfecho.de/api/health`. Renamed **`UNTESTED-195-20260428-0238-push-to-master.md`** → **`TESTING-195-20260428-0238-push-to-master.md`** at start of this tester run.
...
| Criterion | Result | Evidence |
|-----------|--------|----------|
| (1) Git | **PASS** | **`git rev-parse origin/master origin/development`** → **`7a2c2bd59b2cfb6cbc6a55ac407993494b17256f`**, **`896ec66775fe0ef2fa766be121d8feb235c78ecd`**. **`git merge-base --is-ancestor origin/master origin/development`** → exit **0**. |
| (2) **Deploy to amvara9** green for latest **`master`** | **FAIL** | **`gh run list --repo satisfecho/pos --workflow "Deploy to amvara9" --limit 12`**: top **`master`** run is still **`24773000757`**, **`conclusion`** **`failure`**, **`updatedAt`** **`2026-04-22T10:18:20Z`**. **`gh run view 24773000757 --repo satisfecho/pos --json conclusion,status,updatedAt,headBranch,url,displayTitle`** → **`conclusion: failure`**, **`status: completed`**, **`headBranch: master`**. No newer successful **`master`** deploy after this run. Run URL: https://github.com/satisfecho/pos/actions/runs/24773000757 |
| (3) Optional live after green deploy | **N/A** | Criterion **(2)** not green; **`curl -sS https://satisfecho.de/api/health`** → **200** **`{"status":"ok"}`** (does not prove CI deploy for **`7a2c2bd`**). |

**Overall:** **FAIL** — criterion **(2)**.

**Product owner feedback:** **`origin/master`** remains at promoted commit **`7a2c2bd`**; **`origin/development`** is ahead at **`896ec667`**. **Deploy to amvara9** on **`master`** has still not produced a **green** run after push **`24773000757`** — same blocker as documented in the implementation summary (marketing artifact token / PAT). **Loop protection:** Repeated verification does not change outcome until repository **Actions** secrets are fixed and a **green** workflow run exists (or **Testing instructions §4** manual deploy is documented as satisfying the release). **Next step:** configure **`MARKETING_ARTIFACT_TOKEN` / `GH_TOKEN`** per **`config/marketing-sites.json`**, then **Re-run failed jobs** or push **`master`**; return task as **`UNTESTED-…`** when ready to re-verify.

**URLs tested:**  
1. `https://github.com/satisfecho/pos/actions/runs/24773000757`  
2. `https://satisfecho.de/api/health` (HTTP 200 — informational only)

**Relevant log excerpts:** **`gh run view 24773000757 --repo satisfecho/pos --json conclusion,status,updatedAt,headBranch,url`** → **`{"conclusion":"failure","displayTitle":"Agent 001: add FEAT task for GitHub issue #195 (push to master)","headBranch":"master","status":"completed","updatedAt":"2026-04-22T10:18:30Z","url":"https://github.com/satisfecho/pos/actions/runs/24773000757"}`**.
