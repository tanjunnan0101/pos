...
**Handoff (`012-feature-coder-handoff.md`, 2026-04-30):** **No `wip → untested` rename** while the task is in **testing → wip** after **FAIL** (criterion **(2)**). Coder **git** work is in **Implementation summary**; **Testing instructions** at EOF. Re-handoff to **`UNTESTED-…`** only after the **Deploy to amvara9** blocker is fixed or **§4** manual parity is documented. See **Testing instructions (EOF)**. GitHub **#195** is **closed**; see handoff for label cleanup.
...
**012 handoff (`012-feature-coder-handoff.md`, 2026-04-30):** **No `wip → untested` rename.** Task remains **`WIP-195-20260428-0238-push-to-master.md`** per **`TASKS-README.md`** (**testing → wip** after verification **FAIL** on criterion **(2)**; latest reports unchanged — **`Deploy to amvara9`** run **`24773000757`** still **failure**). Coder / operator: fix **`MARKETING_ARTIFACT_TOKEN` / `GH_TOKEN`**, re-run deploy, or document **§4** manual **`scripts/deploy-amvara9.sh`** parity; then **wip → untested** again. GitHub issue **#195** is **closed**; removed **`agent:wip`** and **`agent:untested`** from the issue (`gh issue edit 195 --remove-label …`) so agent labels match closed state. Tester picks up from **`UNTESTED-…`** only after that rename.
