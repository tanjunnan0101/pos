...
**Handoff (`012-feature-coder-handoff.md`, 2026-04-30):** After **`./scripts/git-sync-development.sh`**, coder implementation is complete per **Implementation summary** (git promotion done; CI blocked on Actions secrets until PAT/secrets or **§4** manual deploy). **Testing instructions** are in this file (canonical block near the end, after historical test reports). Renamed **`WIP-195-20260428-0238-push-to-master.md`** → **`UNTESTED-195-20260428-0238-push-to-master.md`**; GitHub issue **#195**: **`agent:untested`** added, **`agent:wip`** removed. **Tester:** rename **`UNTESTED-…` → `TESTING-…`** when verification starts; criterion **(2)** may still **FAIL** until secrets + green **`master`** **Deploy to amvara9** or documented **§4** parity.
