...
- **Ollama (local):** **`cursor-agent`** is still required for steps that edit **`back/`**, **`front/`**, and run tools inside Cursor. Local models cannot replace that without a separate “apply patch + run tests” runner. What *does* help is **cheap triage** so **`cursor-agent`** runs less often for **001** when only noisy log heuristics fired:
  - **On by default** when **`ollama`** is on **`PATH`**, the daemon responds, and **`ollama list`** shows at least one model. Set **`AGENT_001_OLLAMA_LOG_TRIAGE=0`** to force-disable. Script: **`scripts/agent-ollama-log-triage.sh`** (reads the digest; **`OLLAMA_MODEL`** defaults to **`qwen2.5:1.5b`**). It only runs when **Docker heuristics** would trigger **001** and **there are zero untracked GitHub issues** (issues path unchanged — no missed **FEAT** from local LLM).
