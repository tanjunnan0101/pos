...
- **Local LLM (llama.cpp → Ollama):** **`cursor-agent`** is still required for steps that edit **`back/`**, **`front/`**, and run tools inside Cursor. Local models cannot replace that without a separate “apply patch + run tests” runner. What *does* help is **cheap triage** so **`cursor-agent`** runs less often for **001** when only noisy log heuristics fired:
  - **On by default** when **`scripts/agent-ollama-log-triage.sh`** can run: it **first** calls a **llama.cpp** server with an OpenAI-compatible API (default **`http://127.0.0.1:8080/v1`**, model **`Bonsai-8B.gguf`** via **`LLAMA_CPP_BASE_URL`** / **`LLAMA_CPP_MODEL`**), using **`python3`** for the HTTP POST. If that fails or returns nothing, it **falls back to Ollama** (**`ollama run`**, **`OLLAMA_MODEL`** default **`qwen2.5:1.5b`**). Triage is considered available if **`GET`** the same base URL with **`/models`** appended (e.g. **`http://127.0.0.1:8080/v1/models`**) succeeds and **`python3`** exists, **or** **`ollama list`** shows at least one model. Set **`AGENT_001_OLLAMA_LOG_TRIAGE=0`** to force-disable. Set **`AGENT_001_SKIP_LLAMA_CPP=1`** to skip the llama.cpp attempt and use only Ollama. It only runs when **Docker heuristics** would trigger **001** and **there are zero untracked GitHub issues** (issues path unchanged — no missed **FEAT** from local LLM).
  - **Ollama fallback picks (examples from `ollama list`):** **`qwen2.5:1.5b`**, **`nemotron-3-nano:4b`**, **`huihui_ai/granite3.2-abliterated:2b`** for speed; **`qwen3.5:4b`** / **`gemma3:latest`** if you want a bit more judgment; **`qwen2.5-coder:latest`** only if you later add code-shaped local steps (heavier).
