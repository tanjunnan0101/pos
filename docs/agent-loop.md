...
- **Local LLM (llama.cpp → Ollama):** **`cursor-agent`** is still required for steps that edit **`back/`**, **`front/`**, and run tools inside Cursor. Local models cannot replace that without a separate “apply patch + run tests” runner. What *does* help is **cheap triage** so **`cursor-agent`** runs less often for **001** when only noisy log heuristics fired:
  - **On by default** when **`scripts/agent-ollama-log-triage.sh`** can run: it **first** calls a **llama.cpp** server with an OpenAI-compatible API (default **`http://127.0.0.1:8080/v1`**, model **`Bonsai-8B.gguf`** via **`LLAMA_CPP_BASE_URL`** / **`LLAMA_CPP_MODEL`**), using **`python3`** for the HTTP POST. **Ollama** (**`ollama run`**, **`OLLAMA_MODEL`** default **`Gemma4:latest`**) runs **only** if the llama.cpp step yields **no** assistant text (HTTP failure, empty content, or missing **`choices[0].message.content`**). If llama.cpp returns **non-empty** text that lacks **`ESCALATE`** / **`SKIP`**, the script does **not** call Ollama and **escalates** (conservative). The loop passes **`OLLAMA_MODEL`** into the triage script so logs match the model in use. **`OLLAMA_HOST`** defaults to **`http://127.0.0.1:11434`** only when **unset**; an environment **`OLLAMA_HOST`** set before the run is **not** overridden (remote daemon possible). Set **`AGENT_001_LOG_TRIAGE_DEBUG=1`** to print **stderr** from the llama.cpp **`python3`** call and from **`ollama run`** (otherwise stderr is discarded). Triage is considered available if **`GET`** the same base URL with **`/models`** appended (e.g. **`http://127.0.0.1:8080/v1/models`**) succeeds and **`python3`** exists, **or** **`ollama list`** shows at least one model (**`OLLAMA_HOST`** respected for the list check in **`agents2/pos-cursor-loop.sh`**). Set **`AGENT_001_OLLAMA_LOG_TRIAGE=0`** to force-disable. Set **`AGENT_001_SKIP_LLAMA_CPP=1`** to skip the llama.cpp attempt and use only Ollama. It only runs when **Docker heuristics** would trigger **001** and **there are zero untracked GitHub issues** (issues path unchanged — no missed **FEAT** from local LLM).
