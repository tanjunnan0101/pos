...
- **Committer (`agents2/pos-cursor-loop.sh`):** by default **`AGENT_COMMITTER_LOCAL`** is on: if the **only** dirty paths under repo root are **`agents2/001-gh-reviewer/time-of-last-review.txt`**, the loop runs **`git commit`** + **`git pull --rebase`** + **`git push origin development`** itself (**no `cursor-agent`**). Any other change (e.g. **`CHANGELOG.md`**, **`front/`**, **`.gitignore`**) skips **`cursor-agent`** unless **`AGENT_COMMITTER_USE_CURSOR=1`**. Set **`AGENT_COMMITTER_LOCAL=0`** to always use the **`040-committer.md`** **`cursor-agent`** path when there are uncommitted changes. Judgment for changelog text and SemVer bumps still belongs to humans or **`cursor-agent`** when **`AGENT_COMMITTER_USE_CURSOR=1`**.
