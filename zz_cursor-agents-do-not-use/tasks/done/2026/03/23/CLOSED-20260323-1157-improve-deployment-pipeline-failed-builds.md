...
## GitHub
- **Issue:** https://github.com/tanjunnan0101/pos/issues/49
...
- Wrong **`git remote origin`** (not containing **`tanjunnan0101/pos`**) exits **1** unless **`SKIP_ORIGIN_CHECK=1`**.
...
- **(Server / staging only)** Run **`SKIP_ORIGIN_CHECK=1 bash scripts/deploy-amvara9.sh`** from a checkout whose **`origin`** is intentionally not sakario if you must validate forks; production amvara9 should use default origin check.
...
4. **Results:**
   ...
   - **Origin guard + `SKIP_ORIGIN_CHECK`:** **PASS** — L52–L68: URL must match `*tanjunnan0101/pos*` or exit 1; bypass when `SKIP_ORIGIN_CHECK=1`.
