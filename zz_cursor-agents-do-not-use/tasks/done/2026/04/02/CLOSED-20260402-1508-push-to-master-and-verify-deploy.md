...
- **What was tested:** Tester confirmed the deploy workflow green, **https://www.sakario.sg/** and **/api/health** healthy, and **origin/master** and **origin/development** both contain commit **357990b** as required.
...
- **Issue:** https://github.com/tanjunnan0101/pos/issues/152
...
1. **GitHub Actions:** Open https://github.com/tanjunnan0101/pos/actions — confirm the latest **Deploy to amvara9** run for the push to **`master`** is **green** (run id **23907796006** or newer on the same commit).
2. **Production smoke (optional spot-check):** From a browser or `curl`, verify **https://www.sakario.sg/** returns **200**, **`/api/health`** returns healthy JSON, and landing **app version** / meta matches expectations (workflow already hit these URLs).
...
3. **What was tested:** Per **Testing instructions** — **`deploy-amvara9`** run **23907796006**; **`https://www.sakario.sg/`** and **`/api/health`**; **`origin/master`** and **`origin/development`** contain promotion commit **`357990b`**.
...
4. **Results**
   ...
   | Production **`/`** and **`/api/health`** | **PASS** | `curl -w '%{http_code}' https://www.sakario.sg/` → **200**; `curl https://www.sakario.sg/api/health` → `{"status":"ok"}` |
...
7. **URLs tested**
   ...
   1. `https://github.com/tanjunnan0101/pos/actions/runs/23907796006`
   2. `https://www.sakario.sg/`
   3. `https://www.sakario.sg/api/health`

8. **Relevant log excerpts**

   ```
   $ gh run list --repo tanjunnan0101/pos --workflow=deploy-amvara9.yml --limit 3
   completed  success  …  Deploy to amvara9  master  push  23907796006  …
   ```

   ```
   $ curl -sS -o /dev/null -w "%{http_code}" https://www.sakario.sg/
   200
   $ curl -sS https://www.sakario.sg/api/health
   {"status":"ok"}
   ```
